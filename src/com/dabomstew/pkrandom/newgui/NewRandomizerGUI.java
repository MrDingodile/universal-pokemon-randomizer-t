...
public class NewRandomizerGUI {
    ...
    private Settings createSettingsFromState(CustomNamesSet customNames) {
        ...
        settings.setRivalCounterType(tpRivalTypeCheckbox.isSelected());
        ...
        boolean normalHMOnly = tmFullNormalHMCompatibilityCheckBox.isSelected() && tmFullNormalHMCompatibilityCheckBox.isVisible();
        settings.setFullHMCompat(!normalHMOnly && tmFullHMCompatibilityCheckBox.isSelected() && tmFullHMCompatibilityCheckBox.isVisible());
        settings.setFullNormalHMCompat(normalHMOnly);
    ...
    private void InitStat(JComponent j){
        j.setVisible(true);
        j.setEnabled(false);
    }
    private void InitStat(AbstractButton j){
        InitStat((JComponent)j);
        j.setSelected(false);
    }
    private void InitStat(JComboBox j){
        InitStat((JComponent)j);
        j.setSelectedIndex(0);
    }
    private void InitStat(JSlider j, int v){
        InitStat((JComponent)j);
        j.setValue(v);
    }
    private void InitStat(JSpinner j, int v){
        InitStat((JComponent)j);
        j.setValue(v);
    }
    ...
    private void initialState() {
        ...
        InitStat(limitPokemonCheckBox);
        InitStat(limitPokemonButton);
        InitStat(noIrregularAltFormesCheckBox);
        InitStat(raceModeCheckBox);

        currentRestrictions = null;

        InitStat(openROMButton);
        InitStat(randomizeSaveButton);
        InitStat(premadeSeedButton);
        InitStat(settingsButton);
        openROMButton.setEnabled(true);
        randomizeSaveButton.setEnabled(true);
        premadeSeedButton.setEnabled(true);
        settingsButton.setEnabled(true);

        InitStat(loadSettingsButton);
        InitStat(saveSettingsButton);
        InitStat(pbsUnchangedRadioButton);
        InitStat(pbsShuffleRadioButton);
        InitStat(pbsRandomRadioButton);
        InitStat(pbsLegendariesSlowRadioButton);
        InitStat(pbsStrongLegendariesSlowRadioButton);
        InitStat(pbsAllMediumFastRadioButton);
        InitStat(pbsStandardizeEXPCurvesCheckBox);
        InitStat(pbsEXPCurveComboBox);
        pbsEXPCurveComboBox.setModel(new DefaultComboBoxModel<>(new String[] { "Medium Fast" }));
        InitStat(pbsFollowEvolutionsCheckBox);
        InitStat(pbsUpdateBaseStatsCheckBox);
        InitStat(pbsFollowMegaEvosCheckBox);
        InitStat(pbsUpdateComboBox);
        pbsUpdateComboBox.setModel(new DefaultComboBoxModel<>(new String[] { "--" }));
        InitStat(pbsAssignEvoStatsRandomlyCheckBox);
        InitStat(ptUnchangedRadioButton);
        InitStat(ptRandomFollowEvolutionsRadioButton);
        InitStat(ptRandomCompletelyRadioButton);
        InitStat(ptFollowMegaEvosCheckBox);
        InitStat(ptIsDualTypeCheckBox);
        pokemonAbilitiesPanel.setVisible(true);
        InitStat(paUnchangedRadioButton);
        InitStat(paRandomRadioButton);
        InitStat(paAllowWonderGuardCheckBox);
        InitStat(paFollowEvolutionsCheckBox);
        InitStat(paTrappingAbilitiesCheckBox);
        InitStat(paNegativeAbilitiesCheckBox);
        InitStat(paBadAbilitiesCheckBox);
        InitStat(paFollowMegaEvosCheckBox);
        InitStat(paWeighDuplicatesTogetherCheckBox);
        InitStat(paEnsureTwoAbilitiesCheckbox);
        InitStat(peUnchangedRadioButton);
        InitStat(peRandomRadioButton);
        InitStat(peRandomEveryLevelRadioButton);
        InitStat(peSimilarStrengthCheckBox);
        InitStat(peSameTypingCheckBox);
        InitStat(peLimitEvolutionsToThreeCheckBox);
        InitStat(peForceChangeCheckBox);
        InitStat(peChangeImpossibleEvosCheckBox);
        InitStat(peMakeEvolutionsEasierCheckBox);
        InitStat(peRemoveTimeBasedEvolutionsCheckBox);
        InitStat(peAllowAltFormesCheckBox);
        InitStat(spUnchangedRadioButton);
        InitStat(spCustomRadioButton);
        InitStat(spRandomCompletelyRadioButton);
        InitStat(spRandomTwoEvosRadioButton);
        InitStat(spTypeRadioButton);
        InitStat(spComboBox1);
        InitStat(spComboBox2);
        InitStat(spComboBox3);
        ...
        InitStat(spRandomizeStarterHeldItemsCheckBox);
        InitStat(spBanBadItemsCheckBox);
        InitStat(spAllowAltFormesCheckBox);
        InitStat(stpUnchangedRadioButton);
        InitStat(stpSwapLegendariesSwapStandardsRadioButton);
        InitStat(stpRandomCompletelyRadioButton);
        InitStat(stpRandomSimilarStrengthRadioButton);
        InitStat(stpPercentageLevelModifierCheckBox);
        InitStat(stpPercentageLevelModifierSlider);
        InitStat(stpLimitMainGameLegendariesCheckBox);
        InitStat(stpRandomize600BSTCheckBox);
        InitStat(stpAllowAltFormesCheckBox);
        InitStat(stpSwapMegaEvosCheckBox);
        InitStat(stpFixMusicCheckBox);
        InitStat(igtUnchangedRadioButton);
        InitStat(igtRandomizeGivenPokemonOnlyRadioButton);
        InitStat(igtRandomizeBothRequestedGivenRadioButton);
        InitStat(igtRandomizeNicknamesCheckBox);
        InitStat(igtRandomizeOTsCheckBox);
        InitStat(igtRandomizeIVsCheckBox);
        InitStat(igtRandomizeItemsCheckBox);
        InitStat(mdRandomizeMovePowerCheckBox);
        InitStat(mdRandomizeMoveAccuracyCheckBox);
        InitStat(mdRandomizeMovePPCheckBox);
        InitStat(mdRandomizeMoveTypesCheckBox);
        InitStat(mdRandomizeMoveCategoryCheckBox);
        InitStat(mdUpdateMovesCheckBox);
        InitStat(mdUpdateComboBox);
        mdUpdateComboBox.setModel(new DefaultComboBoxModel<>(new String[] { "--" }));
        InitStat(pmsUnchangedRadioButton);
        InitStat(pmsRandomPreferringSameTypeRadioButton);
        InitStat(pmsRandomCompletelyRadioButton);
        InitStat(pmsMetronomeOnlyModeRadioButton);
        InitStat(pmsGuaranteedLevel1MovesCheckBox);
        InitStat(pmsReorderDamagingMovesCheckBox);
        InitStat(pmsNoGameBreakingMovesCheckBox);
        InitStat(pmsForceGoodDamagingCheckBox);
        InitStat(pmsGuaranteedLevel1MovesSlider, pmsGuaranteedLevel1MovesSlider.getMinimum());
        InitStat(pmsForceGoodDamagingSlider);
        pmsForceGoodDamagingSlider.setValue(pmsForceGoodDamagingSlider.getMinimum());
        InitStat(pmsEvolutionMovesCheckBox);
        InitStat(tpComboBox);
        tpComboBox.setModel(new DefaultComboBoxModel<>(new String[] { "Unchanged" }));
        InitStat(tpRivalCarriesStarterCheckBox);
        InitStat(tpSimilarStrengthCheckBox);
        InitStat(tpWeightTypesCheckBox);
        InitStat(tpDontUseLegendariesCheckBox);
        InitStat(tpNoEarlyWonderGuardCheckBox);
        InitStat(tpRandomizeTrainerNamesCheckBox);
        InitStat(tpRandomizeTrainerClassNamesCheckBox);
        InitStat(tpForceFullyEvolvedAtCheckBox);
        InitStat(tpForceFullyEvolvedAtSlider, tpForceFullyEvolvedAtSlider.getMinimum());
        InitStat(tpPercentageLevelModifierSlider, 0);
        InitStat(tpPercentageLevelModifierCheckBox);

        InitStat(tpEliteFourUniquePokemonCheckBox);
        InitStat(tpEliteFourUniquePokemonSpinner, 1);

        InitStat(tpAllowAlternateFormesCheckBox);
        InitStat(tpSwapMegaEvosCheckBox);
        InitStat(tpDoubleBattleModeCheckBox);
        InitStat(tpBossTrainersCheckBox);
        InitStat(tpImportantTrainersCheckBox);
        InitStat(tpRegularTrainersCheckBox);
        InitStat(tpBossTrainersSpinner, 1);
        InitStat(tpImportantTrainersSpinner, 1);
        InitStat(tpRegularTrainersSpinner, 1);
        tpAdditionalPokemonForLabel.setVisible(true);
        tpHeldItemsLabel.setVisible(true);
        InitStat(tpBossTrainersItemsCheckBox);
        InitStat(tpImportantTrainersItemsCheckBox);
        InitStat(tpRegularTrainersItemsCheckBox);
        InitStat(tpConsumableItemsOnlyCheckBox);
        InitStat(tpSensibleItemsCheckBox);
        InitStat(tpHighestLevelGetsItemCheckBox);
        InitStat(tpRandomShinyTrainerPokemonCheckBox);
        InitStat(tpBetterMovesetsCheckBox);
        totpPanel.setVisible(true);
        totpAllyPanel.setVisible(true);
        totpAuraPanel.setVisible(true);
        InitStat(totpUnchangedRadioButton);
        InitStat(totpRandomRadioButton);
        InitStat(totpRandomSimilarStrengthRadioButton);
        InitStat(totpAllyUnchangedRadioButton);
        InitStat(totpAllyRandomRadioButton);
        InitStat(totpAllyRandomSimilarStrengthRadioButton);
        InitStat(totpAuraUnchangedRadioButton);
        totpAuraUnchangedRadioButton.setSelected(true);
        InitStat(totpAuraRandomRadioButton);
        InitStat(totpAuraRandomSameStrengthRadioButton);
        InitStat(totpPercentageLevelModifierCheckBox);
        InitStat(totpPercentageLevelModifierSlider, 0);
        InitStat(totpRandomizeHeldItemsCheckBox);
        InitStat(totpAllowAltFormesCheckBox);
        InitStat(wpUnchangedRadioButton);
        InitStat(wpRandomRadioButton);
        InitStat(wpArea1To1RadioButton);
        InitStat(wpGlobal1To1RadioButton);
        InitStat(wpARNoneRadioButton);
        InitStat(wpARSimilarStrengthRadioButton);
        InitStat(wpARCatchEmAllModeRadioButton);
        InitStat(wpARTypeThemeAreasRadioButton);
        InitStat(wpUseTimeBasedEncountersCheckBox);
        InitStat(wpDontUseLegendariesCheckBox);
        InitStat(wpSetMinimumCatchRateCheckBox);
        InitStat(wpRandomizeHeldItemsCheckBox);
        InitStat(wpBanBadItemsCheckBox);
        InitStat(wpBalanceShakingGrassPokemonCheckBox);
        InitStat(wpPercentageLevelModifierCheckBox);
        InitStat(wpPercentageLevelModifierSlider, 0);
        InitStat(wpSetMinimumCatchRateSlider, wpSetMinimumCatchRateSlider.getMinimum());
        InitStat(wpAllowAltFormesCheckBox);
        InitStat(wpPercentageTypeCheckBox);
        InitStat(wpPercentageTypeComboBox);
        wpPercentageTypeComboBox.setModel(new DefaultComboBoxModel<>(new String[] { "Normal" }));
        InitStat(wpPercentageTypeSlider, wpPercentageTypeSlider.getMinimum());

        InitStat(tmUnchangedRadioButton);
        InitStat(tmRandomRadioButton);
        InitStat(tmNoGameBreakingMovesCheckBox);
        InitStat(tmFullHMCompatibilityCheckBox);
        InitStat(tmFullNormalHMCompatibilityCheckBox);
        InitStat(tmLevelupMoveSanityCheckBox);
        InitStat(tmKeepFieldMoveTMsCheckBox);
        InitStat(tmForceGoodDamagingCheckBox);
        InitStat(tmForceGoodDamagingSlider, tmForceGoodDamagingSlider.getMinimum());
        InitStat(tmFollowEvolutionsCheckBox);
        InitStat(thcUnchangedRadioButton);
        InitStat(thcRandomPreferSameTypeRadioButton);
        InitStat(thcRandomCompletelyRadioButton);
        InitStat(thcFullCompatibilityRadioButton);
        InitStat(mtUnchangedRadioButton);
        InitStat(mtRandomRadioButton);
        InitStat(mtNoGameBreakingMovesCheckBox);
        InitStat(mtLevelupMoveSanityCheckBox);
        InitStat(mtKeepFieldMoveTutorsCheckBox);
        InitStat(mtForceGoodDamagingCheckBox);
        InitStat(mtForceGoodDamagingSlider, mtForceGoodDamagingSlider.getMinimum());
        InitStat(mtFollowEvolutionsCheckBox);
        InitStat(mtcUnchangedRadioButton);
        InitStat(mtcRandomPreferSameTypeRadioButton);
        InitStat(mtcRandomCompletelyRadioButton);
        InitStat(mtcFullCompatibilityRadioButton);
        InitStat(fiUnchangedRadioButton);
        InitStat(fiShuffleRadioButton);
        InitStat(fiRandomRadioButton);
        InitStat(fiRandomEvenDistributionRadioButton);
        InitStat(fiBanBadItemsCheckBox);
        InitStat(shUnchangedRadioButton);
        InitStat(shShuffleRadioButton);
        InitStat(shRandomRadioButton);
        InitStat(shBanOverpoweredShopItemsCheckBox);
        InitStat(shBanBadItemsCheckBox);
        InitStat(shBanRegularShopItemsCheckBox);
        InitStat(shBalanceShopItemPricesCheckBox);
        InitStat(shGuaranteeEvolutionItemsCheckBox);
        InitStat(shGuaranteeXItemsCheckBox);
        InitStat(puUnchangedRadioButton);
        InitStat(puRandomRadioButton);
        InitStat(puBanBadItemsCheckBox);
        InitStat(miscBWExpPatchCheckBox);
        InitStat(miscNerfXAccuracyCheckBox);
        InitStat(miscFixCritRateCheckBox);
        InitStat(miscFastestTextCheckBox);
        InitStat(miscRunningShoesIndoorsCheckBox);
        InitStat(miscRandomizePCPotionCheckBox);
        InitStat(miscAllowPikachuEvolutionCheckBox);
        InitStat(miscGiveNationalDexAtCheckBox);
        InitStat(miscUpdateTypeEffectivenessCheckBox);
        InitStat(miscLowerCasePokemonNamesCheckBox);
        InitStat(miscRandomizeCatchingTutorialCheckBox);
        InitStat(miscBanLuckyEggCheckBox);
        InitStat(miscNoFreeLuckyEggCheckBox);
        InitStat(miscBanBigMoneyManiacCheckBox);
        InitStat(tpComboBox);
        InitStat(tpComboBox);
        InitStat(tpComboBox);
    ...
    private void romLoaded() {
        ...
        try {
            ...
            tpRivalTypeCheckbox.setEnabled(true);
    ...
    private void enableOrDisableSubControls() {
        ...
        tpRivalTypeCheckbox.setEnabled(true);
        tpRivalTypeButton.setEnabled(tpRivalTypeCheckbox.isEnabled() && tpRivalTypeCheckbox.isSelected());
        ...
        if (tpRivalTypeCheckbox.isSelected() || !peRandomEveryLevelRadioButton.isSelected() && (!spUnchangedRadioButton.isSelected() || !isTrainerSetting(TRAINER_UNCHANGED))) {
    ...
    private void testRivalTypeStarter(){
        if (!romHandler.isYellow()) {
            Settings settings;
            try {
            ...
            boolean allowAltFormes = romHandler.generationOfPokemon() >= 6;
            List<Pokemon> randomStarters = romHandler.getRivalStarters(settings);
            List<Pokemon> allPokes = romHandler.getPokemon(true, allowAltFormes, false);
