...
public abstract class AbstractRomHandler implements RomHandler {
    ...
    private Type[] typesInGame;
    @Override
    public Type[] getTypesInGame() { return typesInGame; };
    @Override
    public void setTypesInGame(Type[] t) { typesInGame = t; };
    ...
    public void randomEncounters(Settings settings) {
        ...
        boolean useTypePct = settings.useWildType();
        Type type = getTypesInGame()[settings.getWildType()];
        ...
        if (catchEmAll) {
            ...
            for (EncounterSet area : scrambledEncounters) {
                ...
                int e = 0;
                for (Encounter enc : area.encounters) {
                    boolean rollTypeChance = e == typeIndex;
            ...
            for (EncounterSet area : scrambledEncounters) {
                ...
                for (Encounter enc : area.encounters) {
                    boolean rollTypeChance = e == typeIndex;
                    enc.pokemon = pickEntirelyRandomPokemon(allowAltFormes, noLegendaries, area, banned, settings, rollTypeChance);
                    setFormeForEncounter(enc, enc.pokemon);
                    e++;
                }
            }
    ...
    private void area1to1EncountersImpl(List<EncounterSet> currentEncounters, Settings settings) {
        ...
        if (catchEmAll) {
            ...
            for (EncounterSet area : scrambledEncounters) {
                ...
                int typeIndex = this.random.nextInt(inArea.size());
                int p = 0;
                for (Pokemon areaPk : inArea) {
                    boolean rollTypeChance = p == typeIndex;
                    Pokemon picked = pickWildPowerLvlReplacement(localAllowed, areaPk, false, usedPks, 100, settings, rollTypeChance);
                    while (picked.actuallyCosmetic) {
                        picked = pickWildPowerLvlReplacement(localAllowed, areaPk, false, usedPks, 100, settings, false);
                    }
            ...
            for (EncounterSet area : scrambledEncounters) {
                ...
                for (Pokemon areaPk : inArea) {
                    boolean rollTypeChance = p == typeIndex;
                    Pokemon picked = pickEntirelyRandomPokemon(allowAltFormes, noLegendaries, area, banned, settings, rollTypeChance);
                    while (areaMap.containsValue(picked)) {
                        picked = pickEntirelyRandomPokemon(allowAltFormes, noLegendaries, area, banned, settings, false);
                    }
                    areaMap.put(areaPk, picked);
                    p++;
                }
        ...
        while (!remainingLeft.isEmpty()) {
            if (usePowerLevels) {
                ...
                if (remainingRight.size() == 1) {
                    ...
                    pickedRightP = pickWildPowerLvlReplacement(remainingRight, pickedLeftP, true, null, 100, settings, false);
                }
        ...
        for (EncounterSet area : currentEncounters) {
            for (Encounter enc : area.encounters) {
                ...
                if (area.bannedPokemon.contains(enc.pokemon)) {
                    ...
                    if (usePowerLevels) {
                        enc.pokemon = pickWildPowerLvlReplacement(tempPickable, enc.pokemon, false, null, 100, settings, false);
                    } else {
                        int picked = this.random.nextInt(tempPickable.size());
                        enc.pokemon = tempPickable.get(picked);
                    }
    ...
    @Override
    public void onlyChangeWildTypeAndLevels(Settings settings) {
        int levelModifier = settings.isWildLevelsModified() ? settings.getWildLevelModifier() : 0;
        ...
        if (levelModifier != 0) {
            for (EncounterSet area : currentEncounters) {
                ...
                for (Encounter enc : area.encounters) {
                    boolean rollTypeChance = e == typeIndex;
                    if(rollTypeChance && useTypeChance(settings)) {
                        enc.pokemon = getRandomPokemon(false, false, area, null, settings, true);
                        setFormeForEncounter(enc, enc.pokemon);
                    }
                ...
                if (area.displayName.contains("Rock Smash")) {
                    ...
                    if (catchEmAll) {
                        for (Encounter enc : area.encounters) {
                            ...
                            if (shouldRandomize) {
                                enc.pokemon = pickEntirelyRandomPokemon(allowAltFormes, noLegendaries, area, banned, settings, false);
                                setFormeForEncounter(enc, enc.pokemon);
                            }
                        }
                    } else if (typeThemed) {
    ...
    public void randomizeStartersOfType(Type type, Settings settings) {
        ...
        //even with the added range pool is less than the required starters so fill it with very plain normal types
        if(size < starterCount){
            List<Pokemon> normalPadding = new ArrayList<>();
            for (Pokemon pk : allPokes) {
                switch(pk.number){
                    ...
                    case Species.lillipup:
                        normalPadding.add(pk);
                        break;
                }
            }
            for(int i = 0; i < starterCount - size; i++) {
                Pokemon pk = normalPadding.get(random.nextInt(normalPadding.size()));
                unique.add(pk);
                weighted.add(pk);
            }
        }
    ...
    private int CountEvos(Pokemon pk, Type t, Type[] allowedSecondaries) {
        //to do: avoid stack overflow if pk has already been travelled
        if(HasType(pk, t, allowedSecondaries)) {
            if (HasEvolution(pk)) {
                int c = 1, max = -10;
                //recurse to see how far it goes
                for (Evolution ev : pk.evolutionsFrom) {
                    max = Math.max(max, CountEvos(ev.to, t, new Type[0]));
                }
                c += max;
                return c;
            }
    ...
    private Type[] getSecondariesForType(Type type){
        switch (type){
            ...
            case FIRE:
                return new Type[] { Type.FIGHTING };
            case STEEL:
            case GROUND:
                return new Type[] { Type.ROCK };
            case ROCK:
            case WATER:
                return new Type[] { Type.GROUND };
            case ICE:
                return new Type[] { Type.WATER, Type.GROUND, Type.ROCK };
            case POISON:
                return new Type[] { Type.DARK, Type.BUG };
            case FLYING:
                return new Type[] { Type.BUG };
            case ELECTRIC:
                return new Type[] { Type.BUG, Type.STEEL };
            case GHOST:
                return new Type[] { Type.POISON, Type.FLYING };
            case DARK:
                return new Type[] { Type.GROUND, Type.POISON };
            case PSYCHIC:
                return new Type[] { Type.FAIRY };
            case FIGHTING:
    ...
    private boolean useTypeChance(Settings settings){
        int typePct = settings.getWildTypePercentage();
        boolean useType = false;
        if(settings.useWildType()){
            if(this.random.nextInt(100) < typePct) {
                useType = true;
            }
        }
        if(!useType){
    ...
    private Pokemon pickWildPowerLvlReplacement(List<Pokemon> pokemonPool, Pokemon current, boolean banSamePokemon,
            List<Pokemon> usedUp, int bstBalanceLevel, Settings settings, boolean useType) {
        // start with within 10% and add 5% either direction till we find
        // something

        Type type = getTypesInGame()[settings.getWildType()];
        useType = useType && useTypeChance(settings);
        ...
        while (canPick.isEmpty() || (canPick.size() < 3 && expandRounds < 3)) {
            for (Pokemon pk : pokemonPool) {
                if (pk.bstForPowerLevels() >= minTarget && pk.bstForPowerLevels() <= maxTarget
                        ...
                        && !canPick.contains(pk)
                        && (!useType || pk.primaryType == type || pk.secondaryType == type)) {
                    canPick.add(pk);
                }
            }
            minTarget -= currentBST / 20;
            maxTarget += currentBST / 20;
            expandRounds++;
            if(useType && expandRounds >= 20){
                useType = false;
                expandRounds = 0;
                minTarget += currentBST;
                maxTarget -= currentBST;
            }
        }
        return canPick.get(this.random.nextInt(canPick.size()));
    }
    ...
    public Pokemon pickEntirelyRandomPokemon(boolean includeFormes, boolean noLegendaries, EncounterSet area, List<Pokemon> banned, Settings settings, boolean useType) {
        useType = useType && useTypeChance(settings);
        return getRandomPokemon(includeFormes,noLegendaries,area,banned,settings,useType);
    }

    public Pokemon getRandomPokemon(boolean includeFormes, boolean noLegendaries, EncounterSet area, List<Pokemon> banned, Settings settings, boolean useType) {
        Type type = getTypesInGame()[settings.getWildType()];
        Pokemon result;
        ...
        while (result.actuallyCosmetic
           || (useType && result.primaryType != type && result.secondaryType != type) ) {
